<template>
  <section class="canvas-area">
    <svg ref="svgElement" class="graph-svg" :style="svgBackgroundStyles" @click.self="$emit('canvas-click', $event)"
      @mousemove="$emit('on-drag', $event)" @mouseup="$emit('stop-drag')" @mouseleave="$emit('stop-drag')"
      @wheel.prevent="$emit('wheel', $event)" @mousedown.self="$emit('mousedown', $event)">
      <g :transform="`translate(${panX}, ${panY}) scale(${zoomLevel})`">
        <!--         
        <defs>
          <marker id="arrow" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="6" markerHeight="6"
            orient="auto-start-reverse">
            <path d="M 0 0 L 10 5 L 0 10 z" :fill="currentTheme === 'light-theme' ? '#555' : '#ccc'" />
          </marker>
        </defs>
-->
        <!-- Edges-->
        <g v-for="edge in edgesWithCoords" :key="edge.id">
          ...
          <path :d="edge.pathData" stroke="transparent" stroke-width="10" fill="none"
            style="pointer-events: visibleStroke;" @click.stop="$emit('select-element', edge)" />
          <!-- Visible path -->
          <path :d="edge.pathData" :stroke="edge.color" :stroke-width="edge.strokeWidth"
            :stroke-dasharray="edge.strokeDasharray" style="pointer-events: none;"
        ...
        <g v-for="node in nodes" :key="node.id" :transform="`translate(${node.x}, ${node.y})`"
          @mousedown.stop="$emit('start-drag', node, $event)" @click.stop="$emit('node-click', node)" :class="['node-group', node.shape, {
            'selected': selectedElement && selectedElement.id === node.id || edgeStartNode?.id === node.id
          }]">
          <circle v-if="node.shape === 'circle'" :r="getNodeRadius(node)" :fill="node.color" :stroke="node.borderColor"
            stroke-width="2" />
        ...
        <g v-if="selectedElementHandlePos && selectedElementHandlePos.visible">
          <circle :cx="selectedElementHandlePos.x" :cy="selectedElementHandlePos.y" r="8" class="manipulation-handle"
            @mousedown.stop="$emit('start-handle-drag')" />
        </g>

        <!-- Flip button for directed edges -->
        <g v-if="flipButtonPosition && flipButtonPosition.visible"
          :transform="`translate(${flipButtonPosition.x}, ${flipButtonPosition.y})`" @click.stop="$emit('flip-edge')"
          class="flip-handle">
