<template>
    ...
    <div class="graph-editor-container" :class="currentTheme" style="margin-top: 120px;">
        ...
        <Toolbar :canvas-background-style="canvasBackgroundStyle" :canvas-background-color="canvasBackgroundColor"
            :is-zoom-enabled="isZoomEnabled" :current-optimization-mode="optimizationMode" @export-image="exportImage"
            @export-pdf="exportPDF" @export-json="exportJSON" @import-json="triggerImportJSON"
            @set-background="setCanvasBackground" @update-background-color="canvasBackgroundColor = $event"
            @toggle-zoom="toggleZoomMode" @set-optimization-mode="setOptimizationMode" />

        <main class="main-content">
            <Sidebar :is-adding-node="isAddingNode" :node-shape="nodeShape" :is-adding-edge="isAddingEdge"
                :is-eraser-active="isEraserActive" :show-matrix="showMatrix" @toggle-node-mode="toggleNodeMode"
                @toggle-edge-mode="toggleEdgeMode" @toggle-eraser="toggleEraserMode" @toggle-matrix="toggleMatrixView"
                @clear-canvas="clearCanvas" />

            <GraphCanvas ref="graphSvg" :nodes="nodes" :edges-with-coords="edgesWithCoords"
                :selected-element="selectedElement" :edge-start-node="edgeStartNode" :current-theme="currentTheme"
                :svg-background-styles="svgBackgroundStyles" :zoom-level="zoomLevel" :pan-x="panX" :pan-y="panY"
                :selected-element-handle-pos="selectedElementHandlePos" :flip-button-position="flipButtonPosition"
                :is-editing="isEditing" @canvas-click="handleCanvasClick" @node-click="handleNodeClick"
                @select-element="handleElementSelect" @start-drag="startDrag" @on-drag="handleDrag"
                @stop-drag="handleStopDrag" @start-handle-drag="startHandleDrag" @flip-edge="flipSelectedEdgeDirection"
                @wheel="handleWheel" @mousedown="handleMouseDown" />

            <NodeEditBox v-if="selectedElement?.type === 'node' && isEditing" :node="selectedElement"
                :zoom-level="zoomLevel" :pan-x="panX" :pan-y="panY" @close="isEditing = false" />

            <EdgeEditBox v-if="selectedElement?.type === 'edge' && isEditing" :edge="selectedElement"
                :position="getEdgeValuePosition()" :get-node-label="getNodeLabel" @close="isEditing = false"
                @update-direction="updateEdgeDirection" />
        </main>

        <MatrixModal v-if="showMatrix" :nodes="nodes" :adjacency-matrix="adjacencyMatrix" :current-theme="currentTheme"
...
.light-theme .graph-editor-container {
    background-color: rgba(247, 243, 240, 1);
    color: #333;
}

.dark-theme .graph-editor-container {
