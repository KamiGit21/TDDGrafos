  ...
  <div class="dijkstra-modal-overlay" @click.self="closeModal">
    <div :class="['dijkstra-modal-content', theme]">
      <header class="dijkstra-modal-header">
        <h2>Algoritmo de Dijkstra (Camino Máximo)</h2>
      ...
      <main class="dijkstra-modal-body" v-if="nodes.length > 0">
        <div class="node-selection-panel">
          <div class="selection-group">
            ...
            <select id="startNode" v-model="selectedStart" @change="calculateDijkstraMax">
          ...
          <div class="selection-group">
            ...
            <select id="endNode" v-model="selectedEnd" @change="calculateDijkstraMax">
              <option value="">Seleccionar...</option>
              <option v-for="node in nodes" :key="node.id" :value="node.id">
                {{ node.label }}
              </option>
        ...
        <div class="info-panel" v-if="selectedStart && selectedEnd && pathExists">
          <p><strong>Distancia máxima:</strong> {{ Math.round(maxDistance * 100) / 100 }}</p>
          <p><strong>Camino máximo:</strong> {{ maxPath.join(' → ') }}</p>
        </div>
        ...
        <svg class="graph-svg" ref="graphSvgDijkstra" @wheel.prevent="handleWheel" @mousedown.self="startPan" @mousemove="onDrag" @mouseup="stopDrag" @mouseleave="stopDrag">
          <g :transform="`translate(${panX}, ${panY}) scale(${zoomLevel})`">
            ...
            <g v-for="edge in edgesWithCoords" :key="edge.id">
              <path
                :d="edge.pathData"
                :stroke="edge.isInPath ? '#f59e0b' : (edge.color || edgeDefaultColor)"
                :stroke-width="edge.isInPath ? '3' : '2'"
                fill="none"
                :marker-end="'url(#arrow-dijkstra)'"
                :class="{ 'max-path': edge.isInPath }"
              />
              <text
                :x="edge.textX"
                :y="edge.textY"
...
const maxPathNodeFill = computed(() => props.theme === 'light-theme' ? 'rgba(245, 158, 11, 0.15)' : 'rgba(251, 191, 36, 0.25)');

const calculateDijkstraMax = () => {
  ...
  // Algoritmo de camino máximo usando Bellman-Ford modificado
  const dist = new Map();
  const prev = new Map();
  ...
  // Relajar todas las aristas N-1 veces (para camino máximo)
  const n = props.nodes.length;
  
  for (let i = 0; i < n - 1; i++) {
    let updated = false;
    
    // Procesar cada nodo que tenga una distancia válida
    props.nodes.forEach(node => {
      const nodeId = node.id;
      const currentDist = dist.get(nodeId);
      
      // Solo procesar si el nodo es alcanzable
      if (currentDist !== -Infinity) {
        ...
        for (const neighbor of neighbors) {
          ...
          // Actualizar si encontramos un camino más largo
          if (newDist > neighborDist) {
            dist.set(neighbor.to, newDist);
            prev.set(neighbor.to, { nodeId: nodeId, edgeId: neighbor.edgeId });
            updated = true;
          }
        }
      }
    ...
    // Optimización: si no hubo actualizaciones, terminar antes
    if (!updated) break;
  }
...
const maxDistance = computed(() => {
  ...
  const dist = distances.value.get(selectedEnd.value);
  return dist === -Infinity ? 0 : dist;
});
...
const getNodeStroke = (node) => {
  ...
  if (node.isInPath) return '#f59e0b';
...
const exportJSON = () => {
  const fileName = prompt("Ingresa el nombre del archivo para guardar:", "grafo_dijkstra_max");
  if (!fileName) return;
